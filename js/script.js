(function($) {
	...
	if ($('.nav-overlay').length) {
		...
		TweenMax.to({}, 0.016, {
			repeat: -1,
			onRepeat: function() {
				// Only show cursor when sidebar is open (not when close-sidebar class is present)
				if (!$('.hidden-sidebar').hasClass('close-sidebar')) {
				posX += (mouseX - posX) / 9;
				posY += (mouseY - posY) / 9;
		...
		$("button, a").on("mouseenter", function() {
			// Only show cursor effects when sidebar is open
			if (!$('.hidden-sidebar').hasClass('close-sidebar')) {
		...
		$(".nav-overlay").on("mouseenter", function() {
			// Only show close cursor when sidebar is open
			if (!$('.hidden-sidebar').hasClass('close-sidebar')) {
	...
	if ($('.cause-carousel').length) {
		var causeCarousel = new Swiper('.cause-carousel', {
			spaceBetween: 0,
			loop: true,
			speed: 1000,
			autoplay: 5000,
